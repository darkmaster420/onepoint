...
const config = {
    ...
    THEMES: ['w.w.art', 'simple.art', 'dark'],
    MODULES: ['node_fs', 'onedrive', 'coding', 'teambition', 'googledrive', 'alidrive', 'phony'],
};

const configTemplate = {
    html: '<div>This is a custom html, you can also place some script files here</div>',
    logo: 'https://cdn.onesrc.cn/uploads/images/onepoint_30.png',
    name: 'DemoSite',
    readme: `## Deployment successful

    Congratulations on the successful deployment, but this does not mean that the system can be used
    
    Next, you need to enter the [admin](/admin/) page and complete some necessary configurations
    
    Please note that some platform configuration parameters may require you to configure them yourself on the platform.
    
    After the configuration is complete, you can add the cloud disk
`,
    cors: ['*'],
    proxy: [],
...
const commonMParams = [
    ...
    _P('hidden', [], 'The file or folder you currently want to hide, e.g. /images/today, /video/something.mp4', 2, '', false, false),
];
...
api_router.get('config/basic', async (ctx) => {
    ...
    if (op.needConfig) {
        [
            P('admin_salt', String(Math.random()), 'Signature salt can only be entered once and cannot be changed later.', 9, '', false, false),
            P('admin_pass', 'admin', 'password', 9, '', false, true),
            P('admin_name', 'admin', 'User name can only be entered once and cannot be changed later.', 9, '', false, true),
...
// Obtain all cloud disk information and add optimistic locking to make up for the problem that the token cannot recognize multiple logins.
...
module.exports = async (ctx, next) => {
    ...
    // Solve the problem of url unchanged under multiple baseURL caused by cache
...
module.exports = {
    get params() {
        return [
            _P('type', 'com', 'com:Global cn:Chinese', 7, ['com', 'cn'], false, true),
...
const e = {
    ...
    Unauthorized({ field, type }) {
        ...
        if (type === 'empty') {
            ...
            s = 'Is invalid, please enter and try again';
        } else if (type === 'wrong') {
            s = 'There is an error, please retry';
        }
        return `字段${field}${s}`;
    },
    CommandNotAllowed({ command }) {
        return `${command}is not supported yet`;
    },
    DriveNotExist({ path }) {
        return `path ${path} is not configured under any cloud disk`;
    },
    ModuleNotExist({ module }) {
        return `module ${module} does not exist`;
    },
    InvalidPage({ page }) {
        return `page parameters${page}are not allowed`;
    },
    ItemIsFile({ path }) {
...
var globalThis = typeof window !== 'undefined' ? window : typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : {};
...
$$out+="<!DOCTYPE html><html lang=\"en-us\"><head><meta charset=\"utf-8\"><link rel=\"shortcut icon\" href=\""
...
$$out+="<!DOCTYPE html><html lang=\"en-us\"><head><meta charset=\"utf-8\"><meta name=\"github\" content=\"https://github.com/ukuq/onepoint\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1,shrink-to-fit=no\"><link rel=\"shortcut icon\" href=\""
$$out+=$escape($V.site.logo)
...
// Buttons
$$out += "<script src=\"https://cdn.staticfile.org/clipboard.js/2.0.11/clipboard.min.js\"></script><div class=\"input-group\"><input type=\"url\" class=\"form-control\" id=\"op-share-url\"><div class=\"input-group-append\"><button type=\"button\" class=\"btn btn-outline-secondary\" id=\"op-share-btn\" data-clipboard-target=\"#op-share-url\" data-clipboard-action=\"cut\">Copy Link</button>"
$$out += "<a type=\"button\" class=\"btn btn-outline-secondary\" href=\"" + url + "\" target=\"_blank\">Download File</a>"
$$out += "<a type=\"button\" class=\"btn btn-outline-secondary\" href=\"" + vUrl + "\" target=\"_blank\">Play With VLC</a></div></div><div class=\"border rounded my-3 p-3\" id=\"op-file\">"
// Initialize Clipboard.js 
$$out += "<script>new ClipboardJS(\"#op-share-btn\");</script>"
if(type === 'image'){
$$out+=" <img src=\"" + url + "\" class=\"rounded mx-auto d-block img-fluid\" max-width=\"100%\" alt=\"Image failed to load\">"
// If Video
}else if(type === 'video'){
// Initialize Vidstack
$$out+="<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/vidstack/styles/defaults.min.css\" /><link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/vidstack/styles/community-skin/video.min.css\" /><script type=\"module\" src=\"https://cdn.jsdelivr.net/npm/vidstack/dist/cdn/prod.js\"></script>"
// Embed Player
$$out+="<media-player title=\"" + fName + "\" crossorigin> <media-outlet> <source src=\"" + oUrl + "\" type=\"video/mp4\" /> </media-outlet> <media-community-skin></media-community-skin></media-player>"
// If Mkv
}else if($V.file.name.endsWith('.mkv')){
// Initialize Vidstack
$$out+="<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/vidstack/styles/defaults.min.css\" /><link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/vidstack/styles/community-skin/video.min.css\" /><script type=\"module\" src=\"https://cdn.jsdelivr.net/npm/vidstack/dist/cdn/prod.js\"></script>"
// Embed Player
$$out+="<media-player title=\"" + fName + "\" crossorigin> <media-outlet> <source src=\"" + oUrl + "\" type=\"video/mp4\" /> </media-outlet> <media-community-skin></media-community-skin></media-player>"
$$out+="<div>File is mkv you may have issues with audio or video. If you do click \"Play With VLC\""
// If file is m3u8
}else if($V.file.name.endsWith('.m3u8')){
// Embed Player 
$$out+="<div id=\"op-preview-video\" data-url=\"" + url + "\"></div>"
// import dependancies with cdn
$$out+="<script src=\"//cdn.staticfile.org/hls.js/1.0.4/hls.min.js\"></script><script src=\"//cdn.staticfile.org/flv.js/1.5.0/flv.min.js\"></script><script src=\"//cdn.staticfile.org/dplayer/1.26.0/DPlayer.min.js\"></script><script>dpElement=document.getElementById(\"op-preview-video\"),window.dp=new DPlayer({element:dpElement,video:{url:dpElement.getAttribute(\"data-url\"),pic:\"\",type:\"auto\"}})</script> "
}else if(type === 'audio' || $V.file.name.endsWith('.flac')){
$$out+="<link rel=\"stylesheet\" href=\"https://cdn.plyr.io/3.7.8/plyr.css\" /><audio id=\"player\" controls autoplay><source src=\"" + url + "\" type=\"audio/mp3\" /></audio>"
$$out+="<script src=\"https://cdn.plyr.io/3.7.8/plyr.js\"></script>"
$$out+="<script>const player = new Plyr('#player');</script>"
}else if(type === 'office'){
...
"use strict";
module.exports = JSON.parse('{"author":"ukuq","bugs":{"url":"https://github.com/ukuq/onepoint/issues"},"dependencies":{},"description":"a tiny file index and manage program","devDependencies":{"@vercel/ncc":"^0.28.6","art-template":"^4.13.2","eslint":"^7.16.0","eslint-config-prettier":"^7.1.0","eslint-plugin-prettier":"^3.3.0","html-minifier-terser":"^5.1.1","prettier":"^2.2.1"},"files":["lib"],"homepage":"https://github.com/darkmaster420/onepoint#readme","keywords":["onepoint","onedrive","google-drive","scf","serverless"],"license":"MIT","main":"lib/app.js","name":"onepoint","prettier":{"printWidth":233,"singleQuote":true,"tabWidth":4,"trailingComma":"es5"},"repository":{"type":"git","url":"git+https://github.com/ukuq/onepoint.git"},"scripts":{"__local_start":"node lib/starters/local-test.js","__pre_commit":"node tmp/pre-commit.js","build:ncc":"node ncc/build.js","format":"eslint \\"**/*.js\\" --fix && prettier \\"**/*.{js,json}\\" --write","format:check":"eslint \\"**/*.js\\" && prettier \\"**/*.{js,json}\\" --check","start":"node lib/starters/node-http.js"},"version":"2.0.1","version2":"210620"}');
...
function initApp() {
    ...
    const h = 'Note that you must create a new KV and bind it to the corresponding Workers KV Namespace. The binding name is OPCONFIG';
