...
function signId(p2, id) {
    // 1-10 The annual validity period is purely to make the generated signatures different.
...
module.exports = async (ctx) => {
    ...
    if (ctx.response.isList) {
        ...
        if (e.id) {
            // The documentation requires that when returning file, it must be a canonical path.
            e.id = signId(p2, e.id);
        }
        // Simplify module code duplication
        if (e.mime === undefined) {
