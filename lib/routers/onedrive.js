const { request, RTError } = require('../utils/node');
...
module.exports = {
    get params() {
        return [
            ...
            _P('refresh_token', '', '<a href="https://point.onesrc.cn/oauth2/" target="_blank">get refresh_token (not working) use rclone</a>', 7, '', true, true),
            _P('share_url', '', 'OneDrive share link', 7, 'https://{?}-my.sharepoint.com/:f:/g/personal/{?}/{?}', false, true),
            _P('root', '', '', 5, '/', false, false),
            _P('api_url', '', 'For sharepoint use this', 5, 'https://graph.microsoft.com/v1.0/sites/{site-id}/drive/', false, false),
    ...
    async handle(config, data, cache, ctx) {
        // The logic is a bit bloated. In order to unify all onedrives
        if ((cache.etime || 0) < Date.now()) {
            ...
            // Control, delete useless attributes type defaults to com
            if (config.type !== 'cn') {
